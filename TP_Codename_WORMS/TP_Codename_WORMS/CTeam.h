...
public:
	...
	@discussion Voilà.
	*/
	CTeam(string _strTeamName, SDL_Color _Color,SDL_Texture* _Texture, Uint8 _uiNbWorms, CFont* _Font){
		m_boFocus = false;
		m_uiWormTurn = 0;
		...
		for (int i = 0; i < _uiNbWorms; i++)
		{
			...
			m_pTabWorm[i] = new CWorm(strNom, new CSprite("", _Texture, 1, 1, 50, 0), _Font, { 0, 0, 30, 40 });
			strNom.pop_back();
		}
